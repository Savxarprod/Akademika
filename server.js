...
if (!ADMIN_TOTP_SECRET) {
  console.error("❌ Не задана переменная окружения ADMIN_TOTP_SECRET");
  process.exit(1);
}
...
app.post("/api/auth/login", loginLimiter, async (req, res) => {
  try {
    ...
    // Если пароль верный, но код не введён — просим ввести
    if (!totpCode) {
      return res.json({ success: false, require2FA: true, message: "Введите код из приложения" });
    }
    ...
    const verified = speakeasy.totp.verify({
      ...
      encoding: 'base32',
      token: totpCode,
      window: 2 // допуск ±60 секунд
    });

    if (!verified) {
      return res.status(401).json({ success: false, require2FA: true, message: "Неверный код 2FA" });
    }
